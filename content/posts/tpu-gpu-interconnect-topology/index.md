...
2D Torus 的问题在于**扩展性**：对于 N 颗芯片，网络直径（最远两点之间的跳数）约为 2×√N。4,096 颗芯片意味着最坏情况下需要约 128 跳——这对集合通信的延迟是灾难性的。
...
3D Torus 将网络直径从 2×√N 降至 3×∛N。同样 4,096 颗芯片，最远跳数从约 128 降到约 48——延迟降低了近 3 倍。更关键的是，**bisection bandwidth（对分带宽）提升了 2-4 倍**，这直接决定了 AllReduce 等集合通信操作的吞吐量。
...
| **域内跳数** | 1 跳 (任意 GPU 等距) | 多跳 (最远 3×∛N 跳) |
