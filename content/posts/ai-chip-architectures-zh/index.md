...
<p>该密度由 <em><strong><a data-popup="passive-copper-backplane">无源铜</a></strong></em>保持在一起。 NVL72 的 NVLink 结构通过背板盲插运行超过 5,184 条电缆（每个机架约 2 英里的布线，没有 <a data-popup="in-cable-retimer">电缆内重定时器</a>， <a data-popup="serdes">class="notranslate"&gt;ZXQSPLITQXZ&lt;/span&gt;SerDes</a> 运行在 GPU 上， <a data-popup="switch-asic">交换机 ASIC</a> 本身），承载约 130 TB/s 的 <a data-popup="all-to-all">所有</a> 跨 72 个 GPU 的带宽。 NVIDIA 估计，与每个链路上都需要 <a data-popup="pluggable-transceiver">可插拔收发器</a> 的光学等效方案相比，选择铜缆可以为每个机架节省大约 20 kW 的电量。铜使得 <em>机架作为一个 GPU</em> 经济实用：在低于 2 米的运行中，它仍然在功耗、成本和信号完整性方面胜出。除此之外，这些位必须安装在玻璃上。</p>
...
<p>有了 Rubin，光学层就崩溃了进入 <a data-popup="switch-asic">交换机 ASIC</a>。 <em><strong><a data-popup="quantum-x-photonics">Quantum-X Photonics</a></strong></em> (<a data-popup="infiniband">InfiniBand</a>）和 <em><strong><a data-popup="spectrum-x-photonics">Spectrum-X 光子学</a></strong></em> （<a data-popup="spectrum-x">以太网</a>）用 <em><strong><a data-popup="cpo">共封装光学器件</a></strong></em>替换可插拔器件：激光器、调制器和光电探测器通过 TSMC COUPE 粘合到开关封装上。 NVIDIA 声称，与 OSFP 可插拔同等产品相比，激光器数量减少约 4 倍，链路功率降低约 3.5 倍。将 GPU 转变为两芯片封装并在其旁边堆叠 HBM 的小芯片逻辑现在出现在网络层：计算、内存、 <em>和</em> 光子学垂直集成在一个基板上。</p>
...
<p><img alt="Zoom 中的一个 TensorCore — 顶部的标量单元在每个周期将 322 位 VLIW 捆绑发射到 8 个功能槽中：VPU 通过其 2D 矢量通道运行逐元素数学运算； XLU 和 Transpose/Permute 单元处理跨车道减少和布局洗牌；四个 256×256 MXU 执行收缩 matmul。累加器队列将部分总和放入 VMEM，这是一个软件管理的暂存器，为阵列提供数据和耗尽数据。" loading="lazy" src="images/google-tpu-tensorcore.png"/></p>
