...
<p>TPU 的 multi-chip execution 采用 SPMD：所有芯片运行同一个 program，但各自处理不同 shard，并通过 hierarchical collective 交换数据。GSPMD（现在逐步由 Shardy 取代）会根据用户在 Mesh 和 PartitionSpec 中给出的少量 sharding annotation，把 partitioning 传播到整张 graph，并在需要的位置插入 all-reduce、all-gather 和 reduce-scatter。如果 compiler 选择的 collective 或 layout 不理想，开发者可以使用 shard_map 编写显式 per-device code，手动指定 local shape 和 collective，同时仍然保留外围的 automatic partitioning。这与 FSDP/DeepSpeed 的抽象边界不同：后者主要在 runtime 和 module boundary 上包装 model，GSPMD/Shardy 则由 compiler 对 whole graph 做 partition。</p>
...
<p>PyTorch 确实可以运行在 TPU 上，但目前不是 TPU 的主力编程栈，成熟度和覆盖面都低于 JAX。<strong><a data-popup="torch-xla">torch_xla</a></strong> 使用 LazyTensor 记录 PyTorch operation，并在 step boundary 把 graph 交给 XLA 编译；compiled artifact 会按 graph shape hash 缓存。PyTorch/XLA 2.x 已加入 GSPMD-style sharding annotation、<code class="notranslate" translate="no">torch.compile</code> backend 和 JAX bridge，graph capture 的开销也明显下降。不过，JAX primitive 与 StableHLO 的映射更直接，对复杂 parallel strategy 的支持也更完整，因此 TPU 上的大规模 training 和 inference stack 仍以 JAX/XLA 为中心。</p>
...
<p>现实限制是 native UALink switch silicon 要到 2027 年才会大规模供货。Astera Labs Scorpio，以及 Auradine、Enfabrica、Xconn 的同类产品，预计在 2026 年末至 2027 年部署。Helios 初期因此使用 UALoE——把 Infinity Fabric tunnel 到标准 Ethernet 上——作为过渡方案，在等待 native UALink fabric 的同时保留 programming model。也就是说，首发版 Helios 更接近高速 Ethernet-tunneled coherent cluster，而不是 NVL72 那种原生 NVLink coherent domain；这是 AMD 为了按时在 2026 年推出 rack-scale product 所做的取舍。</p>
<h5 id="scale-out">横向扩展</h5>
...
<h4 id="software">软件栈</h4>
<p><strong><a href="https://rocm.docs.amd.com/">ROCm</a></strong> 是 AMD 面向 <strong><a href="https://docs.nvidia.com/cuda/cuda-c-programming-guide/">CUDA</a></strong> 提供的开源软件栈。NVIDIA 采用专有且垂直集成的模式：cuBLAS、cuDNN、TensorRT-LLM 等关键组件主要由 NVIDIA 独立开发和维护，其中不少以 binary library 的形式发布。ROCm 则以 GitHub 为主要协作平台，并优先接入 PyTorch、Triton、vLLM、OCP MX 等开放项目和标准，而不是围绕一套由单一厂商控制的专有库构建封闭生态。AMD 与 NVIDIA 之间的软件成熟度和性能仍有差距；AMD 的策略是借助 upstream project 与开源社区逐步缩小差距，而不是从头复制一套与 CUDA 平行的完整生态。</p>
...
<p>PyTorch 是 ROCm 的主力支持路径。Eager-mode PyTorch 很早就已支持 ROCm；<code class="notranslate" translate="no">torch.compile</code> 通过 Triton 生成 kernel，Triton ROCm backend 也已经进入 upstream。ROCm 不采用 XLA 式 whole-graph IR，而是直接 lower 到 HIP、Triton 或 Composable Kernel。随着 Triton 成为 PyTorch 的常用 kernel path，许多 <code class="notranslate" translate="no">torch.compile</code> kernel 可以在 CUDA 与 ROCm 之间复用。AMD 的设计取舍是让 Triton Python DSL 成为跨厂商的公共 kernel language，从而降低 CUDA ecosystem 带来的迁移成本。</p>
...
<p>ROCm 的性能差距仍然存在。独立 benchmark 显示，在标准 PyTorch、vLLM 和 SGLang workload 上，ROCm 7.2 通常比同级 CUDA stack 慢约 10–25%。ROCm 已经接近 feature parity，但还没有达到 performance parity。差距在最新 research kernel 上最明显，例如依赖 Blackwell 新 primitive 的 FlashAttention-4 尚无直接 ROCm implementation，需要等待 AITER、HipKittens 或其他 hand-written kernel 补齐。常见的 Llama inference、attention 和 dense Transformer training 已能较好运行，但新的 long-tail workload 仍会带来额外 porting 和 tuning 成本。</p>
<hr/>
...
<p>Cerebras 的低延迟 inference 性能已经得到独立 benchmark 验证。Artificial Analysis 在 Llama 3.1 8B、70B、405B 以及 Llama 4 Maverick 上测得数百到数千 token/s 的单用户 decode speed，其中部分结果明显高于同期 GPU service。厂商公布的峰值通常更高，尤其会叠加 speculative decoding，因此比较时应区分 independent measurement 与 vendor peak。可以确认的是：在 batch-one、每用户 decode latency 这一指标上，Cerebras 确实处于领先位置。</p>
<p>但这种速度伴随着显著的 system cost。每片 wafer 只有 44 GB SRAM，frontier-scale model 往往需要多台 CS-3 才能容纳 weight 和 KV Cache；decode 时大量 compute unit 处于空闲，真正受限的是 SRAM capacity 与 bandwidth。Cerebras 没有公开 batch size 和 per-system aggregate throughput，公开 API 的 token price 通常也高于主流 GPU provider。长 context 还会让 KV Cache 与 weight 争夺同一片 SRAM，因此需要更多 system 才能维持 model replica。换句话说，Cerebras 优化的是极低 latency，而不是最低的 per-token cost。</p>
...
<p>因此，WSE 找到的是一个边界清晰的市场：它在 batch-one decode latency 上优势明显，并且已有客户愿意为速度支付溢价；但 per-token cost、SRAM capacity、model size 和 customer concentration 都限制了适用范围。WSE 把 memory wall 视为 packaging 问题，并用整片 46,225 mm² wafer 消除了 die-to-die 边界，这是它最独特的价值，也是它最难扩展的约束。</p>
<hr/>
